...
@export var defence_container : HFlowContainer
...
func initialize(inventory : UIInventory, inventory_effects : FightInventoryInstance):
	...
	defence_container.add_child(new_visual)
...
func update_visuals(inventory : UIInventory, fight_inventory_instance : FightInventoryInstance):
	...
	for defence in fight_inventory_instance.defences:
		if !defence_container.has_node(defence.name):
			var new_visual = load("res://scenes/templates/status_effect_template.tscn").instantiate()
			new_visual.get_child(0).modulate = defence.get_color()
			new_visual.get_child(1).texture = load("res://assets/textures/icons/" + defence.name + ".png")
			new_visual.name = defence.name
			defence_container.add_child(new_visual)
		var visual = defence_container.get_node(defence.name)
		visual.get_child(0).text = str(int(ceil(defence.value)))
	
	for defence_visual in defence_container.get_children():
		if defence_visual.name == "health":
			defence_visual.get_child(0).text = "%d/%d" % [ceil(fight_inventory_instance.current_health), ceil(fight_inventory_instance.health)]
		elif fight_inventory_instance.defences.find_custom(func(s): return s.name == defence_visual.name) == -1:
			defence_visual.queue_free()

	var block = fight_inventory_instance.get_defence_value("block")
	var defence_sum = fight_inventory_instance.health + block

	health_bar.size.x = 192.0 * fight_inventory_instance.health / defence_sum
	...
	block_bar.position.x = health_bar.size.x
	...
	for bar in [health_bar, current_health_bar, block_bar, fire_bar, poison_bar, shock_bar, doom_bar]:
		bar.size.x = ceil(bar.size.x)

func clear_visuals():
	for bar in [current_health_bar, health_bar, poison_bar, fire_bar, shock_bar, doom_bar]:
		bar.size.x = 0.0
	...
	for child in defence_container.get_children():
		child.queue_free()
